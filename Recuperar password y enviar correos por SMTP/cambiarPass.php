...
$sel="SELECT * FROM usuarios where pin = '$pin'";
...
if (mysqli_num_rows($res) > 0){
    if($pass == $confirmarPass){
        ...
        $sql = "UPDATE usuario_login set password = '$encoded_pass' where id_usuario = '$id'";
        $qry = mysqli_query($con, $sql) or die (mysqli_error($con));
        if($qry){
            /*Si todo sale bien y el usuario reestablece su contraseña
            el pin se eliminará automaticamente*/
            $del = "UPDATE usuarios set pin = '' where id_usuario = '$id'";
